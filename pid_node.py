...
class PIDcontroller(Node):
    def __init__(self):
        ...
        self.len_history = 10
        self.id2target = {6: 'stop', 3: 'red', 2: 'green', 7: 'yellow'}
        ...
        self.targets = {
            lbl: {
                ...
                'slow_distance': 4.0 # Distance threshold to start slowing
            }
    ...
    def stop_at_red(self, target):
        # Verify if the red light has been consistently detected
        target['detected'] = True if np.mean(target['history']) > target['threshold'] else False
        ...
        if target['detected'] and not target['reacted']:
            if target['distance'] < target['min_distance']:
                while True:
                    ackermann_msg = to_ackermann(0.0, self.last_steering_angle)
                    self.publisher.publish(ackermann_msg)  # Send stop command
                    
                    # Continuously check if a green light is detected
                    green_light = self.targets.get('green', None)
                    if green_light and np.mean(green_light['history']) > green_light['threshold']:
    ...
    def slow_at_yellow(self, target, speed_reduction=0.3):
        # Verify if the yellow light has been consistently detected
        target['detected'] = True if np.mean(target['history']) > target['threshold'] else False

        if target['detected'] and not target['reacted']:
            if target['distance'] < target['slow_distance']:
                slow_speed = max(0.1, self.speed - speed_reduction)
                ackermann_msg = to_ackermann(slow_speed, self.last_steering_angle)
                self.publisher.publish(ackermann_msg)  # Send slow command
                # self.get_logger().info(f"Yellow light detected. Slowing down to {slow_speed}.")
                target['reacted'] = True
        if not any(target['history']) and target['reacted']:
            target['reacted'] = False  # Reset reaction flag if the target hasn't been detected in any recent frames
    ...
    def obj_callback(self, msg: PoseStamped):
        ...
        if lbl == 'red':
            self.stop_at_red(data)
        if lbl == 'yellow':
            self.slow_at_yellow(data)
